...
export const SCENARIOS: Record<string, Scenario> = {
  restaurant: {
    ...
    objectives: [
      { id: 'greet', description: 'Greet the waiter', required: true },
      { id: 'order-drink', description: 'Order a beverage', required: true },
      { id: 'order-food', description: 'Order a main course', required: true },
      { id: 'ask-question', description: 'Ask about menu item or recommendation', required: false },
      { id: 'request-bill', description: 'Request the bill', required: true },
    ],
    keyPhrases: ['Quisiera...', 'Me gustaría...', '¿Qué recomienda?', 'La cuenta, por favor'],
    vocabularyHints: ['agua - water', 'vino - wine', 'pollo - chicken', 'pescado - fish', 'cuenta - bill'],
  ...
  airport: {
    ...
    objectives: [
      { id: 'check-in', description: 'Complete check-in process', required: true },
      { id: 'baggage', description: 'Ask about baggage allowance', required: true },
      { id: 'gate', description: 'Ask where your gate is', required: true },
      { id: 'time', description: 'Confirm boarding time', required: false },
    ],
    keyPhrases: ['¿Dónde está...?', 'Mi vuelo a...', '¿Cuántas maletas puedo llevar?', '¿A qué hora sale el vuelo?'],
    vocabularyHints: ['vuelo - flight', 'maleta - suitcase', 'puerta - gate', 'tarjeta de embarque - boarding pass', 'equipaje - baggage'],
  ...
  jobInterview: {
    ...
    objectives: [
      { id: 'introduce', description: 'Introduce yourself professionally', required: true },
      { id: 'experience', description: 'Describe your work experience', required: true },
      { id: 'skills', description: 'Explain your relevant skills', required: true },
      { id: 'ask-question', description: 'Ask a question about the company', required: false },
    ],
    keyPhrases: ['Tengo experiencia en...', 'He trabajado como...', 'Mis habilidades incluyen...', '¿Podría contarme más sobre...?'],
    vocabularyHints: ['experiencia - experience', 'habilidades - skills', 'puesto - position', 'empresa - company', 'equipo - team'],
  ...
  shopping: {
    ...
    objectives: [
      { id: 'greet', description: 'Greet the shop assistant', required: true },
      { id: 'ask-item', description: 'Ask for a specific item', required: true },
      { id: 'size-color', description: 'Specify size or color preference', required: true },
      { id: 'try-on', description: 'Ask to try something on', required: false },
      { id: 'purchase', description: 'Complete the purchase', required: true },
    ],
    keyPhrases: ['Busco...', '¿Tiene esto en talla...?', '¿Puedo probármelo?', '¿Cuánto cuesta?'],
    vocabularyHints: ['talla - size', 'color - color', 'probador - fitting room', 'pagar - to pay', 'precio - price'],
  ...
  doctor: {
    ...
    objectives: [
      { id: 'greet', description: 'Greet the doctor', required: true },
      { id: 'describe-symptoms', description: 'Describe your symptoms', required: true },
      { id: 'answer-questions', description: "Answer the doctor's questions", required: true },
      { id: 'understand-advice', description: "Understand the doctor's advice", required: true },
    ],
    keyPhrases: ['Me duele...', 'Tengo dolor de...', 'Me siento...', '¿Qué debo hacer?'],
    vocabularyHints: ['dolor - pain', 'cabeza - head', 'estómago - stomach', 'fiebre - fever', 'medicina - medicine'],
  ...
  socializing: {
    ...
    objectives: [
      { id: 'introduce', description: 'Introduce yourself', required: true },
      { id: 'ask-about-them', description: 'Ask about the other person', required: true },
      { id: 'share-interests', description: 'Share your interests or hobbies', required: true },
      { id: 'make-plans', description: 'Suggest meeting again', required: false },
    ],
    keyPhrases: ['Me llamo...', '¿De dónde eres?', 'Me gusta...', '¿Quieres tomar un café algún día?'],
    vocabularyHints: ['nombre - name', 'país - country', 'hobbies - hobbies', 'trabajo - work/job', 'estudiar - to study'],
  ...
  business: {
    ...
    objectives: [
      { id: 'present-update', description: 'Present your project update', required: true },
      { id: 'discuss-challenges', description: 'Discuss challenges or obstacles', required: true },
      { id: 'propose-solution', description: 'Propose a solution or next steps', required: true },
      { id: 'answer-questions', description: 'Answer questions from colleagues', required: false },
    ],
    keyPhrases: ['El proyecto está...', 'Hemos completado...', 'Un desafío es...', 'Propongo que...'],
    vocabularyHints: ['proyecto - project', 'equipo - team', 'plazo - deadline', 'presupuesto - budget', 'objetivo - objective'],
  ...
  emergency: {
    ...
    objectives: [
      { id: 'explain-problem', description: 'Explain the emergency or problem', required: true },
      { id: 'provide-details', description: 'Provide necessary details', required: true },
      { id: 'ask-help', description: 'Ask what to do next', required: true },
      { id: 'understand-instructions', description: 'Understand the instructions given', required: true },
    ],
    keyPhrases: ['Necesito ayuda', 'He perdido...', '¿Qué debo hacer?', '¿Dónde puedo...?'],
    vocabularyHints: ['ayuda - help', 'perdido - lost', 'cartera - wallet', 'policía - police', 'documento - document'],
...
export const getScenarioById = (id: string): Scenario | undefined => SCENARIOS[id];
export const SCENARIO_LIST = Object.values(SCENARIOS);
export const getScenariosByDifficulty = (difficulty: DifficultyLevel): Scenario[] =>
  SCENARIO_LIST.filter((s) => s.difficulty === difficulty);
export const getBeginnerScenarios = (): Scenario[] => getScenariosByDifficulty('beginner');
